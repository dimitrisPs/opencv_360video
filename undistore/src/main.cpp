...
float calculateFOV(struct ocam_model *myocam_model);
...
{   
  ...
  /* --------------------------------------------------------------------*/
  double point3D[3] = { 0,-1,0};       // a sample 3D point
...
float calculateFOV(struct ocam_model *myocam_model)
{
	double imgCentrey=myocam_model->yc;
	int width=myocam_model->width;

	double leftimgPoint[2]={0,imgCentrey};
	double rightimgPoint[2]={width,imgCentrey};
	double leftPoint[3];
	double rightPoint[3];
	cam2world(leftPoint, leftimgPoint, myocam_model);
	cam2world(rightPoint, rightimgPoint, myocam_model);
	double thetaleft = atan2(leftPoint[1],leftPoint[0]);
	double thetaright = atan2(rightPoint[1],rightPoint[0]);
	std::cout <<"right "<< thetaright*(180.0/M_PI)<<" left "<<thetaleft* (180.0/M_PI)<<"\tfov "<< (-thetaleft+thetaright)*(180.0/M_PI)<<std::endl;
	return 0;
}
